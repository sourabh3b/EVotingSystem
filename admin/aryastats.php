...
<body>
	...
	<!-- topbar ends -->
		...
		<div class="row-fluid">
			...
			<img src="stpi.jpg" style="width:500px;height;500px;"></img>
...
$abcd=$_SESSION['result'];         //asigning session variable to a variable

$sql="SELECT * FROM post";      //selecting every post
...
$dept=$row['dbpost'];                //selecting dbname of each post
echo "<h4 class='tex' >".$row['post']."</h4>"."<br />";

$sql2="SELECT * FROM `$abcd` where dep='$dept'";
$abc2=mysql_query($sql2);                             //selecting a particular dept from enetrerd hall


$sql3="SELECT * FROM `$abcd` where dep='$dept' ORDER BY vote DESC LIMIT 1"; //finding max number of votes in that particular department
$abc3=mysql_query($sql3);
while($row3=mysql_fetch_array($abc3)){
$winner=$row3['name'];
}
if(mysql_num_rows($abc2)==0){                     //it means no nominations for this category
...
echo "<tr>";
echo "<td>" . $row2['id'] . "</td>";                       //displaying the dta of nominee in the form of table
...
echo "the winner is ". $winner;                  //displaying the winner for each dept
