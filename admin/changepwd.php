...
if(!loggedin()){                                //checking whether logged in or not
...
$dbhall = mysql_query("SELECT * FROM admins WHERE `name` = '$user'");       //checking from database if username exists
while($hallname = mysql_fetch_assoc($dbhall)){
$admin_pwd = $hallname['password'];
}

if(mysql_num_rows($dbhall) > 0 ){
...
<?php  
if(isset($_POST['change'])){                                       //checking if user has submitted form
  	$pass = mysql_real_escape_string ($_POST['pass']);
	$repass = mysql_real_escape_string ($_POST['repass']);
	$newpass = mysql_real_escape_string ($_POST['newpass']);       //assigning entered variables to a new variable
	
	$enc=$pass."".$user;                            //making pattern using password for more security
	$enpwd = md5($enc);                            //encryption of password
	...
	if($pass && $repass && $newpass){
		if($newpass == $repass){
			if(($pass != $newpass)){
				$passcheck = mysql_query("SELECT * FROM `admins` WHERE `name` = '$user' AND `password` = '$enpwd'");             //checking if user with enetered username and password exists
				if(mysql_num_rows($passcheck) > 0){
				$enc2=$newpass."".$user;
	           $newpass=md5($enc2);
					mysql_query("UPDATE `admins` SET `password`= '$newpass' WHERE `name` = '$user' ");               //if exists ,update password
					$a="password changed sucessfully !";
				}else{
				     
					$a="Enter correct password";                         //if u have entered ur current password wrong
				}
			}else{
			      
				$a="newpassword is same as old password";                 //if u have enetered same password for old and new columns
			}
		}else{
		    
			$a="Passwords are not matching";                 //in case if u have enetered passwords different in type and re-type
		} 
	}else{
	     
		$a="enter all fields";                             //incase if u leave any field empty
...
<head>
	...
	<style type="text/css">         <!--styling of body-->
	  body {
		padding-bottom: 40px;
	  }
	  .sidebar-nav {
...
<body>
	...
	<div class="container-fluid">
		...
		<div class="col-md-4">
				...
				<form action="" method="POST">  <!--form for changing the password-->
		<label >Enter Current Password</label>
		<div class="form-group">
		<input type="password" class="form-control" name="pass" placeholder="Enter Current password"  style="height:30px;"/>
		</div>
	
		<label>New Password</label>
		<div class="form-group">
		<input type="password" class="form-control" name="repass" placeholder="Re-Enter Current password" style="height:30px;" />
		</div>
		
		<label>Re-Enter New Password</label>
		<div class="form-group">
		<input type="password" class="form-control" name="newpass" placeholder="Enter New password"  style="height:30px;"/>
		</div>
