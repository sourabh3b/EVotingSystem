...
if(isset($_POST['hall'])){                   //checking if user has submitted form

$abc=$_POST['hall'];
echo $abc;
 $sql="SELECT * FROM `admin` WHERE `htmlname`='$abc'";               //selecting a hall fromdatabase with enetered name
 $ab=mysql_query($sql);
//echo $ab;
while($row=mysql_fetch_assoc($ab)){
$nom=$row['nomhall'];                      //assigning its dbname to a new variable
//echo $nom;
//exit();
}
$_SESSION['result']=$nom;
header('Location:aryastats.php');                //redirecting to stats page
	...
	<div class="container-fluid">
		...
		<div class="col-md-4">
			...
			<form action="" method="POST">
				...
				<button type="submit" class="btn btn-danger form-control" name="delhall" >check results</button>
